...
OA            pass
    ...
    def tearDown(self):
        try:
OA            os.remove("file.json")
        except IOError:
            pass
        try:
OA            os.rename("tmp", "file.json")
        except IOError:
OA            pass
...
OA    def test_two_saves(self):
        pl = Place()
        sleep(0.05)
OA        first_updated_at = pl.updated_at
        pl.save()
OA        second_updated_at = pl.updated_at
        self.assertLess(first_updated_at, second_updated_at)
        sleep(0.05)
        pl.save()
OA        self.assertLess(second_updated_at, pl.updated_at)

OA    def test_save_with_arg(self):
        pl = Place()
OA        with self.assertRaises(TypeError):
            pl.save(None)

OA    def test_save_updates_file(self):
        pl = Place()
OA        pl.save()
        plid = "Place." + pl.id
OA        with open("file.json", "r") as f:
OA            self.assertIn(plid, f.read())


OAclass TestPlace_to_dict(unittest.TestCase):
OA    """Unittests for testing to_dict method of the Place class."""

OA    def test_to_dict_type(self):
OA        self.assertTrue(dict, type(Place().to_dict()))

OA    def test_to_dict_contains_correct_keys(self):
        pl = Place()
        self.assertIn("id", pl.to_dict())
OA        self.assertIn("created_at", pl.to_dict())
        self.assertIn("updated_at", pl.to_dict())
OA        self.assertIn("__class__", pl.to_dict())
...
OAOAOAOA
OA    def test_to_dict_with_arg(self):
        pl = Place()
OA        with self.assertRaises(TypeError):
OAOA            pl.to_dict(None)

OA
OAOA
OAOAif __name__ == "__main__":
OA    unittest.main()
